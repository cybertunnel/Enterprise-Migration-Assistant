...
struct HelperRemote {
    ...
    private func installHelper() throws {
        ...
        guard authStatus == errAuthorizationSuccess else {
            throw MigrationError.helperInstallation("Unable to get a valid empty authorization reference to load Helper daemon")
        }
        ...
        guard authStatus == errAuthorizationSuccess else {
            throw MigrationError.helperInstallation("Unable to get a valid loading authorization reference to load Helper daemon")
        }
        ...
        if SMJobBless(kSMDomainSystemLaunchd, HelperConstants.domain as CFString, authRef, &error) == false {
            ...
            throw MigrationError.helperInstallation("Error while installing the Helper: \(blessError.localizedDescription)")
        }
    ...
    func getRemote() throws -> HelperProtocol {
        ...
        if let unwrappedHelper = helper {
            ...
            throw MigrationError.helperConnection(proxyError?.localizedDescription ?? "Unknown error")
        }
    }
}
